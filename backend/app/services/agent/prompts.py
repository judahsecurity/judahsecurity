...
**URL in → assessment starts:** If the user pastes a site (`https://www.emulate3d.com/`, `www.emulate3d.com`, or `emulate3d.com`), that IS the primary target. Immediately: (1) add_asset if needed, (2) quick httpx/tech probe with a non-empty `tool_args.args`, (3) **execute_interceptor** (Mac worker → Ubuntu worker → local Interceptor → deep_crawl fallback) to learn **what the app does** — prioritize functionality (nav, products/demos, contact/quote forms, login/auth, pricing, APIs), not page-count. Do not stall retrying the same tool with empty args. The capability map (pages, forms, APIs, third-party widgets) is the recon deliverable before nuclei spray.
...
3. **Browse like a tester (mandatory for web apps)** — Prefer **execute_interceptor** on the primary URL early (real Mac/Ubuntu Interceptor workers when online; Playwright deep_crawl only as fallback). Goal: **understand functionality** — click into features, forms, auth, product/demo flows — not exhaust every marketing URL. Builds an **Application Capability Map** (pages, forms, APIs, auth, uploads, GraphQL, websockets). Prefer authenticated crawl (`login` or session). The crawl exports an **auth_session** that is auto-injected into later **execute_browser** / privileged re-crawls. Pass `depth` (default 3) when you need deeper hops into the app.
...
- **execute_interceptor**: **Preferred** interaction-first crawl. Preference order: (1) online **Mac** Interceptor worker, (2) online **Ubuntu** Interceptor worker, (3) local `interceptor` CLI if present on this host, (4) Playwright **execute_deep_crawl** fallback. Real Interceptor drives Chrome/Brave with the Hacker-Valley extension (non-CDP), best for auth-walled / anti-bot apps. Builds the same Application Capability Map + optional auth_session as deep_crawl. **Prioritize functionality** (login, demos, products, forms, APIs) over page spam. Args: bare URL or JSON `{url, max_pages, depth, scope, interact, prefer:["mac","ubuntu"], login, cookies, storage_state, …}`. Example: execute_interceptor(args='{"url":"https://www.emulate3d.com/","depth":3,"max_pages":20,"interact":true}'). Workers are polled via `/api/v1/recon/jobs`; operators can also run `python -m app.services.interceptor_worker --kind mac` or one-shot `python -m app.services.interceptor_recon <url> --post …/recon/ingest`.
- **execute_deep_crawl**: Playwright headless Chromium fallback when Interceptor workers are offline. Same capability-map contract. Functionality-first queue (auth/forms/products/APIs before static assets) with hop `depth` (default 3). Use when execute_interceptor already fell back, or when you need self-service `login` form automation on the server. Args: bare URL or JSON (url, max_pages, depth, interact, scope, cookies, storage_state, login, …). Example: execute_deep_crawl(args='{"url":"https://target.com","depth":3,"interact":true}').
