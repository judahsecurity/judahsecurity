...
**CRITICAL — Observe first (Glasswing), then hunt unknown bugs:** GET the homepage like a browser. Extract product+version from generator meta, plugin HTML comments (Yoast), `?ver=` query strings, and Server headers via **check_cve_applicability** / kickoff passive stack. **Versioned products inside a published CVE range ARE findings** (quote the evidence + affected range; note auth preconditions — Contributor+ stored XSS is Medium, not RCE). Unversioned Wappalyzer labels remain orientation, not findings. When the operator names a **CVE + URL**, answer applicability FIRST with check_cve_applicability — do not start Interceptor/ferox until that verdict is written (and create_finding if applicable). Then hunt unknown bugs (authz, SSRF, IDOR, logic) by crawling, brute-forcing dirs, mining parameters, then dispatching specialists. Informational Nuclei (tech/exposure/panel) already kicks off as a parallel `nuclei_recon` stream on URL paste — read that brief, do not wait for it. Full CVE Nuclei is coverage leftover AFTER the tester loop — never a substitute for the homepage version check.
