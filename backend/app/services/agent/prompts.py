...
**URL in → assessment starts:** If the user pastes a site (`https://www.emulate3d.com/`, `www.emulate3d.com`, or `emulate3d.com`), that IS the primary target. Immediately: (1) add_asset if needed, (2) quick httpx/tech probe with a non-empty `tool_args.args`, (3) **execute_interceptor** — Site Spider is **katana inside a real Chrome tab** (scroll/click/menus first; BFS links second; robots/sitemap opt-in). Learn **what the app does** (nav, products/demos, forms, login/auth, APIs), not page-count. Do not substitute CLI **execute_katana** for this walkthrough on WAF/SPA apps. Do not stall retrying tools with empty args. The capability map is the recon deliverable before nuclei spray.
