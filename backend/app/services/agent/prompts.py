...
3. **Browse like a tester (mandatory for web apps)** — Prefer **execute_interceptor** on the primary URL early (real Mac/Ubuntu Interceptor workers when online; Playwright deep_crawl only as fallback). It clicks safe UI controls, follows links, and builds an **Application Capability Map** (pages, forms, APIs, auth, uploads, GraphQL, websockets). Prefer authenticated crawl (`login` or session). The crawl exports an **auth_session** that is auto-injected into later **execute_browser** / privileged re-crawls.
...
3. **execute_interceptor** on the primary URL (falls back to deep_crawl if no workers). Raise max_pages if the first pass is thin. Review the capability map.
...
- **execute_interceptor**: **Preferred** interaction-first crawl. Preference order: (1) online **Mac** Interceptor worker, (2) online **Ubuntu** Interceptor worker, (3) local `interceptor` CLI if present on this host, (4) Playwright **execute_deep_crawl** fallback. Real Interceptor drives Chrome/Brave with the Hacker-Valley extension (non-CDP), best for auth-walled / anti-bot apps. Builds the same Application Capability Map + optional auth_session as deep_crawl. Args: bare URL or JSON `{url, max_pages, scope, interact, prefer:["mac","ubuntu"], login, cookies, storage_state, …}`. Example: execute_interceptor(args="https://www.emulate3d.com/"). Workers are polled via `/api/v1/recon/jobs`; operators can also run `python -m app.services.interceptor_worker --kind mac` or one-shot `python -m app.services.interceptor_recon <url> --post …/recon/ingest`.
- **execute_deep_crawl**: Playwright headless Chromium fallback when Interceptor workers are offline. Same capability-map contract. Use when execute_interceptor already fell back, or when you need self-service `login` form automation on the server. Args: bare URL or JSON (url, max_pages, interact, scope, cookies, storage_state, login, …). Example: execute_deep_crawl(args="https://target.com").
