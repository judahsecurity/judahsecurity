...
1. `execute_interactsh` register → plant `payload_url` → poll. Do not use Canarytokens.
2. `compare_requests` benign URL vs in-scope canary.
3. Never `169.254.169.254` / localhost if Lictor blocks.
4. OOB DNS without an internal HTTP body is High, not Critical. Poll stdout is the proof.
