...
2. `execute_curl` `GET {origin}/wp-json/wp/v2/users?per_page=100` (`-sS -D-`).
...
3. `compare_requests` POST `{origin}/wp-admin/admin-ajax.php`
...
4. Login oracle: ONE POST `/wp-login.php` per discovered username. Compare
   "not registered" vs "incorrect password". No hydra/rockyou.
5. If a probe is blocked (WAF, 403, timeout): do not stop. Read the defense
...
6. OPTIONAL `execute_wpscan` for plugin CVE names. Skip on abort. Never block
   steps 2–3 on WPScan.
