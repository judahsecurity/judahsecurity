...
def test_injection_and_auth_have_new_arsenal_tools():
    ...
    assert "plan_intruder_mutations" in allowlists["content_api"]
    assert "plan_intruder_mutations" in allowlists["api_authz"]
    assert "run_intruder_batch" in allowlists["api_authz"]
    assert "run_intruder_batch" in allowlists["injection"]
