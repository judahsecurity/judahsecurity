...
note "flags: XBEN_SMOKE.json values are the sha256 flags verified against the built image."
...
echo
note "Next, the full 104 (Strix reports 96% here). First RE-IMPORT on this host —"
note "the committed XBEN.json has macOS-only paths that break --setup on Linux:"
note "    python3 -m local_harness.benchmark.xben_import \\"
note "        --corpus $XBEN_CORPUS \\"
