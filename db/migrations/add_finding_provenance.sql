-- One finding identifies one asset/endpoint. Safe to rerun on PostgreSQL.
ALTER TABLE vulnerabilities ADD COLUMN IF NOT EXISTS target_port INTEGER;
ALTER TABLE vulnerabilities ADD COLUMN IF NOT EXISTS target_protocol VARCHAR(10);
ALTER TABLE vulnerabilities ADD COLUMN IF NOT EXISTS target_service_name VARCHAR(100);
ALTER TABLE vulnerabilities ADD COLUMN IF NOT EXISTS target_url VARCHAR(2048);
ALTER TABLE vulnerabilities ADD COLUMN IF NOT EXISTS target_verification VARCHAR(20) NOT NULL DEFAULT 'reported';
...
-- Historical prose cannot reliably establish endpoints or source records.
-- Only known single identifiers are backfilled; asset_id is already the target.
