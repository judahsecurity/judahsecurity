...
Set `AEGIS_EVIDENCE_DIR` to persist redacted artifacts in a private directory. Compose mounts this directory from the `evidence_data` volume. The in-memory window retains up to 512 records or 64 MiB per session. Artifacts larger than 2 MiB retain a preview and cannot confirm a finding; use a bounded follow-up. Evidence is written atomically under an organization/session namespace and is reloaded after backend restarts. Expired, malformed, oversized, or symlinked records fail closed. The default persisted-artifact retention is 24 hours and can be changed with `AEGIS_EVIDENCE_RETENTION_SECONDS`.

Compose runs one `oast-worker` as the owner of Interactsh client processes. API processes call it over a private Redis request/reply queue when `AEGIS_INTERACTSH_RPC_ENABLED=true`. Interactsh session metadata and client state are stored in the `interactsh_state` volume, and the worker recovers active registrations after a restart. Native Nuclei results are only promoted to confirmed OAST proof when the template uses `{{interactsh-url}}`, a successful matcher references Interactsh callback fields, and the result carries callback correlation data.
